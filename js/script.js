...
const COLOR = {
  ...
  /* Warna per kecamatan batas administrasi */
  admin: {
    "GIRIMULYO":  { stroke: "#f59e0b", fill: "rgba(245,158,11,0.12)" },
    "KALIBAWANG": { stroke: "#06b6d4", fill: "rgba(6,182,212,0.12)"  },
    "SAMIGALUH":  { stroke: "#a855f7", fill: "rgba(168,85,247,0.12)" },
  },
  batasDefault: "#ef4444",
  evac: "#38bdf8",
  route: "#f97316",
...
function styleBatas(f) {
  const ket = (f.properties.ketreranga || "").toUpperCase().trim();
  const c = COLOR.admin[ket] || { stroke: COLOR.batasDefault, fill: "rgba(239,68,68,0.10)" };
  return {
    color: c.stroke,
    weight: 2.5,
    opacity: 0.95,
...
function popupHTML(kind, p) {
  ...
  if (kind === "batas") {
    const ket = p.ketreranga || p.KETRERANGA || "-";
    const c = COLOR.admin[(ket).toUpperCase().trim()] || { stroke: COLOR.batasDefault };
    return `<div class="popup-title">Batas Administrasi</div>` +
      row("Kecamatan", badge(ket, c.stroke)) +
      row("ID", safe(p.Id)) +
      row("Panjang", fmt(p.Shape_Leng, 2) + " m");
  }
  if (kind === "evakuasi") {
 ...
 * ===================================================================== */
/* =====================================================================
 * 12. LEGENDA BATAS ADMINISTRASI DI MAP-STAGE
 * ===================================================================== */
function buildAdminLegend() {
  const el = document.getElementById("adminLegend");
  if (!el) return;

  const entries = Object.entries(COLOR.admin);
  // Judul
  el.innerHTML =
    `<div class="adm-leg-title">` +
    `<span class="adm-leg-dot"></span>Batas Administrasi` +
    `</div>` +
    `<div class="adm-leg-items">` +
    entries.map(([name, c]) =>
      `<div class="adm-leg-item">`+
      `<span class="adm-leg-swatch" style="border-color:${c.stroke};background:${c.fill}"></span>`+
      `<span class="adm-leg-label">${name.charAt(0) + name.slice(1).toLowerCase()}</span>`+
      `</div>`
    ).join("") +
    `</div>`;
}
...
async function main() {
  initMap();
  if (typeof initLaporan === "function") initLaporan(map);
  initUI();
